...
```
The error for 5-fold cross-validation with this seed is close to the error for LOO cross-validation. In fact, it is slightly smaller, though this will not always be the case depending on the value used to seed the pseudo-random number generator used to select the folds.  This is fortunate, as 5-fold cross-validation is far less computationally intensive in general (JWHT Result 5.2 only holds for least squares linear or polynomial regression).
